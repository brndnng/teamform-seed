...
.controller('AdminCtrl', ['$scope', '$firebaseObject', '$firebaseArray', function($scope, $firebaseObject, $firebaseArray) {
	...
	merge_host_name = document.getElementById("merge_team_host");
	merge_guest_name = document.getElementById("merge_team_guest");
  ...
  $scope.merge = function(){
		...
		$scope.TEAMparam.teamName = merge_guest_name;
		var switch_member = $scope.TEAMparam.member.pop();
		if (switch_member == null) {
			break;
		}
		$scope.TEAMparam.teamName = merge_host_name;
		$scope.TEAMparam.member.push(switch_member);
	}
	  $scope.TEAMparam.$save();
	}
